...
def LP_metric(X=None, Y=None, dist=None, delta=0.1, all_res = None, i = 0, j = 0, time_start = NONE):
    ...
    all_res[i,j] = np.array([1-flowProgress[d_ind], cumCost[d_ind], 1-flowProgress[d_1st_ind], APinfo_cleaned[d_1st_ind,4], totalCost, realtotalCost])
...
if __name__ == "__main__":
    ...
    all_res = np.zeros((n,n,6))
    dist = computeDistMatrixGrid(28)
    dist = dist/np.max(dist)

    start_time = time.time()
    Parallel(n_jobs=-1, prefer="threads")(delayed(LP_metric)(mnist_pick[i,:], mnist_pick[j,:], dist, delta, all_res, i, j, start_time) for i in range(n) for j in range(n))
    end_time = time.time()
    print("finish all job in {}s".format(end_time-start_time))
    np.savez('OTP_lp_metric_n_{}'.format(n), all_res=all_res, mnist_pick=mnist_pick, mnist_pick_label=mnist_pick_label)