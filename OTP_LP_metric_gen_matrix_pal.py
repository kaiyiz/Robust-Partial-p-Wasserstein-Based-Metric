...
def LP_metric(X=None, Y=None, dist=None, delta=0.1, all_res = None, i = 0, j = 0):
    ...
    all_res[i,j] = np.array([1-flowProgress[d_ind], cumCost[d_ind], 1-flowProgress[d_1st_ind], APinfo_cleaned[d_1st_ind,4], totalCost])

    # return np.array([1-flowProgress[d_ind], cumCost[d_ind], 1-flowProgress[d_1st_ind], APinfo_cleaned[d_1st_ind,4], totalCost])
...
if __name__ == "__main__":
    ...
    #             if i == j:
    #                 continue
    #             alpha, cost_at_alpha, alpha_dual, dual_at_alpha, total_cost = LP_metric(X=mnist_pick[i,:], Y=mnist_pick[j,:], dist=dist, delta=delta)
    ...
    #             metric_total_cost[i,j] += total_cost
    #         finished[i] = True
    #         end = time.time()
    #         total_time_est = (end-start)*n/(np.count_nonzero(finished)+1)
    #         time_left_est = total_time_est - (end-start)
    #         print("estimate time left: {}s".format(time_left_est))

    tic = time.time()
    Parallel(n_jobs=1, prefer="threads")(delayed(LP_metric)(mnist_pick[i,:], mnist_pick[j,:], dist, delta, all_res, i, j) for i in range(n) for j in range(n))
    toc = time.time()
    ...
    np.savez('OTP_lp_variables_n_{}'.format(n), metric_alpha=metric_alpha, metric_cost_alpha=metric_cost_alpha, metric_alpha_dual=metric_alpha_dual, metric_dual_alpha=metric_dual_alpha, metric_total_cost = metric_total_cost, mnist_pick=mnist_pick, mnist_pick_label=mnist_pick_label)