...
def get_threshold(a, dist, w_ab, p):
    ...
    while noise_level < 1:
        ...
        a_noise = add_noise(a[np.newaxis, :], noise_type = "corner", noise_level=noise_level, range_noise=False)[0,:]
        w_ab_noise = (ot.emd2(a, a_noise, dist))**(1/p)
        if w_ab_noise > w_ab:
            return noise_level
        noise_level += 0.01
    return noise_level

def adding_noise_compare_wass(a, b, dist, p):
    dist_p = dist**p
    wp_ab = (ot.emd2(a, b, dist_p))**(1/p)
    threshold = get_threshold(a, dist_p, wp_ab, p)
    return wp_ab, threshold

def adding_noise_compare_RPW(a, b, dist, delta, k, p):
...
def get_threshold_RPW(a, dist, RPW_ab, delta, k, p):
    ...
    while noise_level < 1:
        # add one empty dimension to a_2, no need to reshape a_2
        a_noise = add_noise(a[np.newaxis, :], noise_type = "corner", noise_level=noise_level, range_noise=False)[0,:]
        RPW_ab_noise = RPW(a, a_noise, dist, delta, k, p)
        if RPW_ab_noise > RPW_ab:
            return noise_level
        noise_level += 0.01
    return noise_level
...
for k in range(N):
    ...
    for i in range(10):
        ...
        for j in range(10):
            if j != i:
                b = X_pick[j*2, :]
                w1_ab, w1_threshold = adding_noise_compare_wass(a_1, b, dist, 1)
                RPW1_ab, RPW1_threshold = adding_noise_compare_RPW(a_1, b, dist, delta=0.01, k=1, p=1)
                w1_a1a2, w1_threshold_a1a2 = adding_noise_compare_wass(a_1, a_2, dist, 1)
                RPW1_a1a2, RPW1_threshold_a1a2 = adding_noise_compare_RPW(a_1, a_2, dist, delta=0.01, k=1, p=1)
                w2_ab, w2_threshold = adding_noise_compare_wass(a_1, b, dist, 2)
                RPW2_ab, RPW2_threshold = adding_noise_compare_RPW(a_1, b, dist, delta=0.0001, k=1, p=2)
                w2_a1a2, w2_threshold_a1a2 = adding_noise_compare_wass(a_1, a_2, dist, 2)
                RPW2_a1a2, RPW2_threshold_a1a2 = adding_noise_compare_RPW(a_1, a_2, dist, delta=0.0001, k=1, p=2)
                w3_ab, w3_threshold = adding_noise_compare_wass(a_1, b, dist, 3)
                RPW3_ab, RPW3_threshold = adding_noise_compare_RPW(a_1, b, dist, delta=0.00005, k=1, p=3)
                w3_a1a2, w3_threshold_a1a2 = adding_noise_compare_wass(a_1, a_2, dist, 3)
                RPW3_a1a2, RPW3_threshold_a1a2 = adding_noise_compare_RPW(a_1, a_2, dist, delta=0.00005, k=1, p=3)
                ...
                np.savetxt("./results/adding_noise_threshold.csv", res, delimiter=",")
                np.savetxt("./results/addint_noise_RPW_threshold.csv", res_RPW, delimiter=",") 
